...
class SharedObjectImplementation {
  ...
  // The C caller only knows that there is an object called SharedObject
  // returned by the C++ instantiator function
public:
...
SharedObject *instantiate_shared_object(const char *name) {
  ...
  // In the instantiate function, we return a reinterpret_cast of Shared Object,
  // making C understands the object
...
void inspect_name(SharedObject *object) {
  ...
  // when we access the object from C, we need to upcast it back to the
  // implementation to access it as the c++ class of the same object
...
void trigger_mutator(SharedObject *object) {
  if (object == nullptr) {
    _print_nonexistent_object();
    return;
  }
